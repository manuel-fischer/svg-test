...
let icons = {};
{
	...
	icons.msgBoxExclamation = function (draw) {
	...
	function draw_icon_paper(draw) {
		draw.setStyle("stroke: #333; fill: #fff");
		draw.polygon([4.5, 7.5],
					[8.5, 3.5],
					[14.5, 3.5],
					[14.5, 16.5],
					[4.5, 16.5]);
		...
		draw.polyline([4.5, 7.5],
					 [8.5, 7.5],
					 [8.5, 3.5]);
	}

	function draw_icon_floppy_disk(draw) {
		draw.setStyle("stroke: #139; fill: #36d");
		draw.polygon([3.5, 3.5],
					[16.5, 3.5],
					[16.5, 16.5],
					[5.5, 16.5],
					[3.5, 14.5]);
	...
	function draw_icon_pencil_overlay(draw) {
		// draw pencil
		// shadow
	...
	icons.newDocument = function (draw) {
		draw_icon_paper(draw);

		draw.setStyle("stroke: #030; fill: #0a0");
		draw.circle(14, 14, 4.75, 0, 2 * Math.PI);
	...
	icons.openDocument = function (draw) {
	...
	icons.saveDocument = function (draw) {
		draw_icon_floppy_disk(draw);
	};

	icons.saveDocumentAs = function (draw) {
		draw_icon_floppy_disk(draw);
		draw_icon_pencil_overlay(draw);
	};
		
	icons.run = function (draw) {
	...
	icons.interrupt = function (draw) {
		draw.setClass("icon-red-fill");
		draw.rect(5, 5, 4, 10);
		draw.rect(11, 5, 4, 10);
	};

	icons.reset = function (draw) {
		draw.setClass("icon-red-fill");
		draw.rect(5, 5, 10, 10);
	};

	
	icons.stepInto = function (draw) {
	...
	icons.breakPoint = function (draw) {
		draw.setClass("icon-red-fill");
		draw.circle(10, 10, 3.9);
	};

	icons.search = function(draw) {
	...
	icons.config = function (draw) {
		...
		for (let i = 0; i < 12; i++) {
			...
			draw.line(10 + 6.0 * Math.cos(a), 10 + 6.0 * Math.sin(a),
					 10 + 9.4 * Math.cos(a), 10 + 9.4 * Math.sin(a));
		}
	};

	icons.restorePanels = function (draw) {
	...
	icons.programView = function (draw) {
		// Outline
	...
	icons.turtle = function (draw) {
		...
		// head
		draw.beginPath();
		draw.ellipse(9.5, 5, 2, 2.5, 0, 0 * Math.PI, 2 * Math.PI, false);
		draw.closePath();
		draw.endPath();
		...
		draw.setStyle("stroke: #070; fill: #2c3");
		draw.beginPath();
		draw.ellipse(9.5, 11.5, 4, 5, 0, 0 * Math.PI, 2 * Math.PI, false);
		draw.closePath();
		draw.endPath();

		draw.setStyle("stroke: #0709; fill: none");
		draw.beginPath();
		draw.ellipse(8.8, 10.8, 4, 5, 0, -0.3 * Math.PI, 0.7 * Math.PI, false);
		draw.endPath();
	};
	
	icons.canvas = function (draw) {
